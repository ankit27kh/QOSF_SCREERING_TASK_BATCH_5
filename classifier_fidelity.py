...
from utilities import get_states, map_cost_fidelity, predict_all_fidelity
import optax
...
# Parameters
layers = 3  # Layers of ansatz
num_qubits = 2  # Number of qubits in ansatz
modify_data = True
epochs = 501
lr = 0.01
...
def fit(params, optimizer, map_cost, X_train, y_train, X_test, y_test, states, circuit):
    """
    Train the model
    :param params: Variational Parameters
    ...
    def step(params, opt_state, X, Y):
        loss_value, grads = jax.value_and_grad(map_cost)(params, X, Y, states, circuit)
    ...
    for i in range(epochs):
        ...
        train_costs.append(map_cost(params, X_train, y_train, states, circuit))
        test_costs.append(map_cost(params, X_test, y_test, states, circuit))
...
plt.tight_layout()
# plt.savefig(f'plots/fidelity_{num_qubits}_{layers}_{modify_data}_cost_plot')
plt.show()

y_predict_train = predict_all_fidelity(
    X_train, variational_parameters, states, classifier_circuit
)
y_predict_test = predict_all_fidelity(
    X_test, variational_parameters, states, classifier_circuit
)
